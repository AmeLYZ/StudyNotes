...
temp = opt.fmin_bfgs(f=cost_func, fprime=grad_func, x0=initial_theta, maxiter=400, full_output=True, disp=False)
theta, cost, unused = temp[0], temp[1], temp[2:]
...
plt.xlabel('Microchip Test 1')
plt.ylabel('Microchip Test 2')
plt.savefig('DecisionBoundaryReg.png', dpi=300)
plt.show()
...
raw_input('ex2_reg Finished. Press ENTER to exit')
